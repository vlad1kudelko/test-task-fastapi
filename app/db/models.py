...
class Organisation(Base):           # Организация - Представляет собой карточку организации в справочнике и должна содержать в себе следующую информацию
    ...
    id_building:  Mapped[int]                                   # Здание: Организация должна находится в одном конкретном здании (Например, Блюхера, 32/1)
    link_org_act                    = relationship('Link_org_act', back_populates='organisation')
...
class Link_org_act(Base):           # Организация может заниматься несколькими видами деятельностей (Например, “Молочная продукция”, “Мясная продукция”)
    ...
    id:           Mapped[int]       = mapped_column(BigInteger, primary_key=True, index=True)
    id_org:       Mapped[int]       = mapped_column(ForeignKey('activity.id'))
    id_act:       Mapped[int]       = mapped_column(ForeignKey('organisation.id'))
    activity                        = relationship('Activity', back_populates='link_org_act')
    organisation                    = relationship('Organisation', back_populates='link_org_act')
...
class Activity(Base):               # Деятельность. Имеет название и может в древовидном виде вкладываться друг в друга
    ...
    link_org_act                    = relationship('Link_org_act', back_populates='activity')
