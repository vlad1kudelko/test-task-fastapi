...
class Organisation(Base):                   # Организация - Представляет собой карточку организации в справочнике и должна содержать в себе следующую информацию
    ...
    id_building:  Mapped[BigInteger | None] = mapped_column(ForeignKey('building.id'))               # Здание: Организация должна находится в одном конкретном здании (Например, Блюхера, 32/1)
...
class Link_org_act(Base):                   # Организация может заниматься несколькими видами деятельностей (Например, “Молочная продукция”, “Мясная продукция”)
    ...
    id:           Mapped[BigInteger]        = mapped_column(BigInteger, primary_key=True, index=True)
    id_org:       Mapped[BigInteger]        = mapped_column(ForeignKey('organisation.id'))
    id_act:       Mapped[BigInteger]        = mapped_column(ForeignKey('activity.id'))
...
class Activity(Base):                       # Деятельность. Имеет название и может в древовидном виде вкладываться друг в друга
    ...
    id:           Mapped[BigInteger]        = mapped_column(BigInteger, primary_key=True, index=True)
    name:         Mapped[str]
    id_parent:    Mapped[BigInteger | None] = mapped_column(ForeignKey('activity.id'))
    link_org_act                            = relationship('Link_org_act')
